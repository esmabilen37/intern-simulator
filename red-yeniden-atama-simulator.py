...
def red_simulasyonu(df, firmalar, red_orani=0.2):
    """
    Yerlesmis ogrencilerin bir kismi reddedilir.
    Reddedilen ogrenciler bosa cikar, ilgili firmanin kontenjani geri acilir.
    """
    yerliler = df[df["Yerlestigi_Firma"].notnull()]
    reddedilecek_sayi = int(len(yerliler) * red_orani)

    if reddedilecek_sayi == 0:
        return df

    reddedilenler = yerliler.sample(reddedilecek_sayi)

    for idx in reddedilenler.index:
        firma_id = df.at[idx, "Yerlestigi_Firma"]

        # ogrenciyi bosa cikar
        df.at[idx, "Yerlestigi_Firma"] = None
        df.at[idx, "Tercih_Sirasi"] = -1

        # firmanin kontenjanini geri ac
        for f in firmalar:
    ...
    return df
print("\n--- YENIDEN ATAMA SIMULASYONU BASLADI ---")

df_sim = sonuc_df.copy()   # ilk greedy sonucu 
iterasyon = 1

while True:
    print(f"\n--- Iterasyon {iterasyon} ---")

    # 1 Firmalar bazi ogrencileri reddeder
    df_sim = red_simulasyonu(df_sim, firmalar, red_orani=0.2)

    # 2 SADECE bosta kalan ogrenciler icin greedy tekrar calissin
    df_sim, sure, islem = greedy_atama(df_sim, firmalar)

    yerlesen = df_sim["Yerlestigi_Firma"].notnull().sum()
    yerlesemeyen = df_sim["Yerlestigi_Firma"].isnull().sum()

    print(f"Yerlesen ogrenci sayisi: {yerlesen}")
    print(f"Yerlesemeyen ogrenci sayisi: {yerlesemeyen}")

    # 3 BITIS SARTI (tum ogrenciler yerlesti mi?)
    if yerlesemeyen == 0:
        print(f"\nTum ogrenciler yerlesti. Simulasyon {iterasyon} iterasyonda tamamlandi.")
        break
