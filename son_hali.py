...
def greedy_atama(df_ogrenciler, firmalar_listesi):
    # Algoritmanın başlangıç zamanını al çalışma süresi ölçümü için
    baslangic_zamani = time.time()
    islem_sayisi = 0
    kontenjan_takibi = {f["Firma_ID"]: f["Kontenjan"] for f in firmalar_listesi}# Firmaların mevcut kontenjanlarını takip etmek için sözlük
   
    df_sirali = df_ogrenciler.sort_values(by="GNO", ascending=False).copy()# Öğrencileri GNO'ya göre büyükten küçüğe sırala
    
    
    for idx, satir in df_sirali.iterrows():# Her öğrenci için sırayla atama yapılır
        for i, tercih in enumerate(satir["Tercihler"]):# Öğrencinin tercih listesi sırayla kontrol edilir
            islem_sayisi += 1
            if kontenjan_takibi[tercih] > 0:# Eğer firmanın kontenjanı varsa yerleştir
...
def normalize_gno(gno): return (gno - 2.0) / 2.0 # GNO değerini 0-1 aralığına normalize eder
def normalize_tercih(ts): return (6 - ts) / 5 # Tercih sırasını 0-1 aralığına normalize eder
def mutluluk_skoru(row):
     # Yerleşmemiş öğrencilerin mutluluğu 0 kabul edilir
    if row["Tercih_Sirasi"] == -1: 
        return 0
    return 0.7 * normalize_gno(row["GNO"]) + 0.3 * normalize_tercih(row["Tercih_Sirasi"])# GNO ve tercih sırası ağırlıklı olarak birleştirilir

def red_simulasyonu(df, firmalar, red_orani=0.2):
    # Yerleşmiş öğrencileri seçiyoruz
    # Yani Yerlestigi_Firma alanı boş olmayan öğrenciler
    # Çünkü sadece yerleşmiş öğrenciler reddedilebilir
    ...
    for idx in reddedilecek.index:
        ...
        df.at[idx, "Tercih_Sirasi"] = -1 #-1 değeri, öğrencinin şu anda yerleşmediğini ifade eder 
        for f in firmalar:
...
def heuristik_zincir_swap(df, iterasyon=1000):
    baslangic = time.time()# Başlangıç zamanı ölçümü
    
    islem = 0 # Yapılan iyileştirme denemesi sayısı baslangicta 0
    df = df.copy() # Orijinal veri bozulmasın diye kopya alınır
   
   
    df["Mutluluk"] = df.apply(mutluluk_skoru, axis=1)# Başlangıç mutluluk skorları hesaplanır
    for _ in range(iterasyon): # Belirlenen sayıda iterasyon boyunca iyileştirme denenir
       
        adaylar = df[(df["Yerlestigi_Firma"].notnull()) & (df["Tercih_Sirasi"] >= 3) & (df["GNO"] < 3.6)]# Geç tercihe yerleşmiş ve orta GNOlu öğrenciler
        
        if len(adaylar) < 3: break #
        i1, i2, i3 = random.sample(list(adaylar.index), 3) #Rastgele 3 öğrenci seçilir (zincir swap)
       
        # Mevcut firmalar ve tercih sıraları saklanır
        ...
        # Zincir swap uygulanır (döngüsel firma değişimi)
        df.at[i1,"Yerlestigi_Firma"], df.at[i2,"Yerlestigi_Firma"], df.at[i3,"Yerlestigi_Firma"] = f2, f3, f1
        
        
        def ts(idx): # Yeni tercih sırasını hesaplayan yardımcı fonksiyon
        ...
        if yeni > eski: # Eğer iyileşme varsa swap kabul edilir
        ...
        else: # Aksi halde eski duruma geri dönülür
            df.at[i1,"Yerlestigi_Firma"], df.at[i2,"Yerlestigi_Firma"], df.at[i3,"Yerlestigi_Firma"] = f1, f2, f3
            df.at[i1,"Tercih_Sirasi"], df.at[i2,"Tercih_Sirasi"], df.at[i3,"Tercih_Sirasi"] = t1, t2, t3
    
    # Güncellenmiş veri, süre, işlem sayısı ve toplam mutluluk döndürülür
...
class EslestirmeArayuzu:
    ...
    def red_sim_calistir(self):
        # Eğer greedy sonucu yoksa, red simülasyonu anlamsızdır
        # Çünkü red işlemi sadece yerleşmiş öğrenciler üzerinde yapılır
        if 'greedy' not in self.sonuclar:
            messagebox.showwarning("Uyarı", "Önce Greedy algoritmasını çalıştırın!")
            return
        
        try:
            # Log ekranına simülasyonun başladığını yazmak için
            self.log("\n=== RED SİMÜLASYONU + YENİDEN ATAMA BAŞLADI ===", COLORS['accent_3'])
            df_sim = self.sonuclar['greedy']['df'].copy()# Greedy sonucunun kopyası alınır
            # Orijinal greedy sonucu bozulmaması için copy() kullanılır
            firmalar_kopya = [f.copy() for f in self.firmalar]# Firma listesi de kopyalanır çünkü kontenjanlar red sırasında değişecek
            
            iterasyon = 0
            # Başlangıç durumu
            toplam_ogrenci = len(df_sim)
            
            while True:#red +yeniden atama süreci
                iterasyon += 1
                #  Firmalar öğrencileri reddeder Red edilen öğrenciler none duruma geçer
                df_sim = red_simulasyonu(df_sim, firmalar_kopya, self.red_orani.get())
                bos_kalan = df_sim["Yerlestigi_Firma"].isnull().sum()# Red sonrası açıkta kalan öğrenci sayısı hesaplanır
                self.log(f"İterasyon {iterasyon}: {bos_kalan} öğrenci şu an açıkta (Red sonrası).")

                #  Açıkta kalanlar için Greedy tekrar çalışır
                df_sim, _, _ = greedy_atama(df_sim, firmalar_kopya)
                
                yerlesen_sayisi = df_sim["Yerlestigi_Firma"].notnull().sum()# Kaç öğrencinin tekrar yerleştiği hesaplanır
                ...
                if iterasyon >= 10: # Sonsuz döngü koruması
                    self.log(f" Uyarı: 10 iterasyon sonunda {df_sim['Yerlestigi_Firma'].isnull().sum()} öğrenci yerleşemedi.", COLORS['warning'])
                    break
            
            df_sim["Mutluluk"] = df_sim.apply(mutluluk_skoru, axis=1)
            toplam_mutluluk = df_sim["Mutluluk"].sum()
            
            self.sonuclar['red'] = { # Red simülasyonu sonuçları kaydedilir
                'df': df_sim,
                'iterasyon': iterasyon,
                'mutluluk': toplam_mutluluk
            }
            
            self.guncelle_tablo(df_sim)   # Tablo ve grafik güncellenir
            self.guncelle_grafik()
            
        except Exception as e:# Herhangi bir hata olursa kullanıcıya göstermek icin
